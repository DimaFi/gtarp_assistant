...
- manual voice toggle: повторный `Ctrl+Alt+A` отменяет capture/STT/preview; hold: key repeat не запускает второй запрос, release немедленно закрывает сегмент; распознанный текст редактируется и отправляется только после подтверждения, а opt-in auto-submit пропускает preview;
