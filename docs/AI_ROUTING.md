...
Неоднозначный FTS-result может пройти локальный semantic rerank. Adapter вызывается только после `Embeddings` resource lease, принимает максимум 16 коротких текстов одним batch, кэширует не более 128 векторов документов и может только переставить уже отфильтрованные статьи. Недоступный endpoint, повреждённый ответ, давление памяти или запущенная GTA оставляют исходный FTS-порядок без ошибки для пользователя.
