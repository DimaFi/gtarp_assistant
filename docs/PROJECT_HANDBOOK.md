...
На текущей точке завершены M1 (opt-in SQLite history), M2 (полный менеджер диалогов и безопасное отображение сообщений) и T1 (версионированный production pipeline benchmark). T1 проверяет 528 сценариев через реальный coordinator и SQLite retrieval; методика и baseline находятся в `PRODUCT_QUALITY_BENCHMARK.md`. Следующий рекомендуемый этап — M3 Push-to-Talk UX: hold/toggle, preview, max duration, microphone test и level meter.

После M3: document-oriented Knowledge UI, затем контролируемый `MemoryService`, кандидаты/подтверждение, профиль и только потом `ContextBuilder`/summaries. Современная визуальная доводка продолжается по `PRODUCT_AND_UI_PLAN.md` и `docs/design`.
