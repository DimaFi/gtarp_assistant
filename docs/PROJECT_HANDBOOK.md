...
Следующий этап — P0.1b: hold-to-talk key-up hook, обнаружение конфликтов горячих клавиш и восстановление microphone unplug/replug. Toggle/cancel, max duration, editable transcript preview, opt-in auto-submit, level meter и тест микрофона уже реализованы и входят в обязательный Release-gate. После P0.1b подключается встроенный STT pack и проверяется полный offline voice knowledge vertical. Современная визуальная доводка продолжается по `PRODUCT_AND_UI_PLAN.md` и `docs/design`, не блокируя автономный сценарий.
