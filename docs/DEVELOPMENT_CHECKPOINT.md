...
- Устранена неоднозначность «Ассистент/Чат»: текущий разговорный экран называется «Чат», а ассистент остаётся названием продукта. Верхняя панель разделяет выбор истории, переименование, повтор, удаление и основное действие «Новый чат».
- Personality controls на странице памяти переведены с ComboBox на четыре snap-slider: подробность, юмор, инициативность и тон. Новый диалог получает compact intent-based title по первому вопросу в RAM и SQLite; ручное переименование сохраняет приоритет.
...
# Текущий checkpoint — автономный голосовой релиз 0.4.1 (15 августа 2026)

Эта секция актуальнее исторических checkpoint ниже.

- Полный светлый UI и светлые compact/expanded overlay проверены на 11 snapshots.
- В основной self-contained ZIP включён русский GigaAM v2/sherpa-onnx STT со своим runtime и проверкой SHA-256.
- Голосовая активация: «Лаберти, слушай», завершение по паузе 1,1 секунды и 12-секундное окно продолжения.
- История чатов сохраняется локально по умолчанию и доступна из шапки ассистента.
- Версия 0.4.1 установлена в `E:\Code\GTA RP Assistant`; `.rollback` содержит предыдущую 0.4.0 и не является исходным деревом.
- Release build: 0 ошибок/предупреждений; все 358 тестов решения прошли.
- Portable ZIP: `artifacts/release/GtaRpAssistant-0.4.1-win-x64.zip`.
- SHA-256: `2d60e8a08e741cf58a6072d8e6fceb11adfbba8fd2c662b19bec774749979fe4`.
- Полная сводка: `docs/RELEASE_NOTES_0.4.1.md`.

Следующий шаг: аппаратный voice QA с реальным микрофоном, шумом GTA, sleep/resume и длительной сессией, затем code signing и обновления.
