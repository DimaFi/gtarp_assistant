...
Актуальный UI-срез: страница разговора в навигации называется «Чат», чтобы не дублировать название продукта «Ассистент». Название диалога генерируется локально и детерминированно по намерению первого вопроса, остаётся редактируемым. Настройки подачи представлены четырьмя дискретными ползунками (0–2); они не влияют на verified facts и grounding.
...
- Долговременная запись не создаётся автоматически: явные предпочтения становятся RAM-кандидатами, проходят secret filter и требуют подтверждения пользователя.
...
- Persistent provenance/source revisions для памяти; базовые RAM-candidates, deduplication, confirmation и relevance search уже реализованы.
...
- [x] bounded RAM-candidates на 24 часа, secret filter, deduplication и ручное подтверждение;
- [ ] persistent `memory_sources`, revisions и summaries;
...
Детерминированное извлечение ограничено явными first-person предпочтениями и просьбами «запомни»; свободный LLM extraction намеренно не используется. Кандидаты остаются только в RAM до подтверждения и истекают через 24 часа. Persistent source provenance и LLM-generated summary старой части диалога пока не выполняются.
